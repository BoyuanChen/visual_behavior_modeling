...
example_measure_imgs_path = './test_results/example_measure_imgs_straight_pos'
    ...
    for p_batch in tqdm(range(num_batch)):
        ...
        for p_data in range(batch_size):
            ...
            if idx == 0:
                ...
                # cv2.circle(data_out_img, (int(x), int(y)), int(radius), (0, 255, 0), 2)
                # cv2.circle(data_out_img, green_center, 5, (0, 255, 0), -1)
                ...
                for p_c in pred_cnts:
                    for p_p in p_c:
                        dis = np.linalg.norm(np.array(p_p) - np.array(green_center))
                        if dis < min_dis:
                            min_dis = dis
                            min_point = p_p
